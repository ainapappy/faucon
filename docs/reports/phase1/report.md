...
- État DB : **le `.env` a été basculé vers MySQL** (`DB_CONNECTION=mysql`, base `faucon@127.0.0.1:3306`) pendant la session d'audit — modification externe conservée. Les tests restent sur sqlite `:memory:` (phpunit.xml). Le schéma complet est portable (migrations standard Blueprint).
